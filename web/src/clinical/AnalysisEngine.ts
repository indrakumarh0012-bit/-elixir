...
const DISEASE_DRUG_RULES: {
  ...
  {
    drugId: /^(doxorubicin|daunorubicin|epirubicin|idarubicin|mitoxantrone|trastuzumab)$/,
    condition: /heart failure|cardiomyopathy/i,
    severity: "High",
    rule: "Anthracyclines and trastuzumab are cardiotoxic — existing heart failure greatly raises the risk of irreversible cardiomyopathy.",
    recommendation: "Cardio-oncology review; baseline and serial echo/GLS; consider liposomal doxorubicin or non-anthracycline regimen.",
  },
  {
    drugId: /^(cisplatin|ifosfamide|pemetrexed|zoledronic-acid|methotrexate)$/,
    condition: /CKD|nephropathy|ESRD/i,
    severity: "High",
    rule: "Nephrotoxic oncology agents on pre-existing kidney disease — high risk of dialysis-level injury.",
    recommendation: "Dose by CrCl or switch (cisplatin→carboplatin AUC dosing; avoid pemetrexed if CrCl < 45; denosumab instead of zoledronic acid).",
  },
  {
    drugId: /^bleomycin$/,
    condition: /ILD|pulmonary fibrosis|COPD/i,
    severity: "High",
    rule: "Bleomycin pulmonary toxicity is more likely and more dangerous on existing lung disease.",
    recommendation: "Baseline PFT with DLCO; consider omitting bleomycin (e.g. AVD instead of ABVD); avoid high FiO2.",
  },
  {
    drugId: /^(nivolumab|pembrolizumab|atezolizumab|durvalumab|ipilimumab)$/,
    condition: /rheumatoid|lupus|IBD|psoriasis|myasthenia|immunosuppressed/i,
    severity: "Moderate",
    rule: "Checkpoint inhibitors can flare pre-existing autoimmune disease.",
    recommendation: "Not an absolute contraindication — co-manage with the specialist; do not stop immunotherapy for mild flares without oncology input.",
  },
  {
    drugId: /^(tamoxifen|megestrol|lenalidomide|thalidomide|pomalidomide)$/,
    condition: /DVT|pulmonary embolism|thrombophilia/i,
    severity: "High",
    rule: "These agents are prothrombotic — prior VTE makes recurrence likely.",
    recommendation: "Anticoagulant/aspirin prophylaxis per protocol (IMiDs), or aromatase inhibitor instead of tamoxifen where suitable.",
  },
  {
    drugId: /^(mercaptopurine|azathioprine)$/,
    condition: /gout|hyperuricemia/i,
    severity: "High",
    rule: "Allopurinol/febuxostat block xanthine oxidase — 6-MP/azathioprine toxicity multiplies.",
    recommendation: "If combined deliberately, cut thiopurine dose to 25% and monitor CBC weekly.",
  },
  {
    drugId: /^(filgrastim|pegfilgrastim|erythropoietin)$/,
    condition: /uncontrolled|active/i,
    severity: "Moderate",
    rule: "Growth factors need indication review.",
    recommendation: "Confirm indication against protocol.",
  },
  {
